...
using MaiFeiPin.Web.UI;
using MaiFeiPin.Common;
using LitJson;
using MaiFeiPin.BLL;


namespace MaiFeiPin.Web.tools
{
    /// <summary>
    /// AJAX提交处理
    /// </summary>
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void comment_list(HttpContext context)
        {
            ...
            article_comment bll = new article_comment();
        ...
        private void user_register(HttpContext context)
        {
            ...
            //检查默认组别是否存在
            Model.user_groups modelGroup = new user_groups().GetDefault();
            if (modelGroup == null)
            {
        ...
        private void order_save(HttpContext context)
        {
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = MaiFeiPin.Web.UI.ShopCart.GetList(user_group_id);
            if (iList == null)
            {
            ...
            //统计购物车
			Model.cart_total cartModel = MaiFeiPin.Web.UI.ShopCart.GetTotal(user_group_id);
            //保存订单=======================================================================
            ...
            //清空购物车
			MaiFeiPin.Web.UI.ShopCart.Clear("0");
            //提交成功，返回URL
